...
public class ClienteServiceImpl implements IClienteService {
	...
	public ClienteDTO save(ClienteDTO cliente) {
		System.out.println("Almacenando cliente: " + cliente.getEmail());
		...
		if (this.servicioAccesoBaseDatos.contarClientesConCorreo(cliente.getEmail()) == 1) {
			ReglaNegocioExcepcion objException = new ReglaNegocioExcepcion(
					"Cliente con correo " + cliente.getEmail() + " existe");
			throw objException;
		}

		ClienteEntity clienteEntity = this.modelMapper.map(cliente, ClienteEntity.class);

		clienteEntity.getObjDireccion().setObjUsuario(clienteEntity);

		clienteEntity.getSolicitudes().forEach(objSolicitud -> objSolicitud.setObjUsuario(clienteEntity));
